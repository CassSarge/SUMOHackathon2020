...
def testing_mode(predictions, frame, width, height):
    ''' Will display the top three predictions for the ASL symbol in the ROI'''

    (prediction_result, pred_2, pred_3) = predictions
...
def demo_mode(target, predictions, frame, width, height):
...
def main():
    ...
    while True:
        ...
        if np.max(my_predict) >= 0.50:
            ...
            predictions = (prediction_result, pred_2, pred_3)

            width = int(video_capture.get(3) + 0.5)
            height = int(video_capture.get(4) + 0.5)

            if (mode):
                testing_mode(predictions, frame, width, height)
            else:
                demo_mode(targetLetter, predictions, frame, width, height)
